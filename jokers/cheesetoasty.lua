SMODS.Joker{ --Cheese Toasty
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and context.game_over == false and context.main_eval  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_9cbd4328', 1, card.ability.extra.odds, 'j_pausespack_cheesetoasty') then
                      temp_played = 0
      ...
      level_up_hand = target_hand}, card)
                        card_eval_status_text(context.blueprint_card or card, 'extra', nil, nil, nil, {message = Upgrade!, colour = G.C.RED})
                  end
            end
        end
    end
